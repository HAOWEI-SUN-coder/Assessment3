...
private:
    ...
    string category; //Housing, Transportation, Food, Utilities, Insurance, Medical, Investing, Entertainment, Miscellaneous
    string description;
...
public:
    ...
    //Constructor.
    Transaction(const string &date, const string &category, const string &description,
            double amount);
...
Transaction::Transaction() :
        date(""), category(""), description(""), amount(0) {

}

Transaction::Transaction(const string &date, const string &category, const string &description,
        double amount) :
...
void Transaction::setCategory(const string &category) {
...
void TransactionList::loadFile(const string &filename) {
    string date; // format: "DD/MM/YYYY"
    string category; //Housing, Transportation, Food
    string description;
    double amount; //positive or negative float value
    ...
    if (ifs.is_open()) {
        ...
        while (getline(ifs, date)) {
            ...
            Transaction trans(date, category, description, amount);
...
void TransactionList::saveFile(const string &filename) const {
    ...
    for (int i = 0; i < size; i++) {
        ofs << transactions[i].getDate() << endl;
        ofs << transactions[i].getCategory() << endl;
        ofs << transactions[i].getDescription() << endl;
        ofs << transactions[i].getAmount() << endl;
    }
    ofs.close();

    cout << "saved " << size << " transactions to " << filename << "." << endl;
}
...
void TransactionList::searchTransaction(const string &keyword) {
    for (int i = 0; i < size; i++) {
        if (transactions[i].getDate().find(keyword) != string::npos
                || transactions[i].getCategory().find(keyword) != string::npos) {
...
void TransactionList::sortTransactions() {
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int j = i; j < size; j++) {
            if (transactions[j].getDateForCompare() > transactions[maxIndex].getDateForCompare()) {
                maxIndex = j;
            }
        }
